...
export const ExamsSubHeader = ({ filterOptions, dropdownFilterOptions, onChangeSearch, onChangeFilter, initFilters, selectedFilters, onSelectFilters }) => {
	...
	return (
		<header className='subHeader' id='examsSubHeader'>
			<input type="text" name="search" className='search' value={search} onChange={(e) => setSearch(e.target.value)} placeholder='Search exams by name...' autoComplete='off' />
									...
									<ul className='optionItemList llh'>
										{filterOptions[opt].map((item) => (
											<li key={item} className="optionItem llh">
												...
												<label className={selectedFiltersTmp[opt].includes(item) ? 'selected' : ''} htmlFor={item.replace(' ', '')}>{item}</label>
											</li>
										))}
									</ul>
								</div>
